...
class Packet:
    ...
    def __init__(self, stream=None, **kwargs):
        if self.id is None or self.contents is None:        # Should be redefined in subclasses
    ...
    def recv(sock: socket.socket):
        ...
        if compressionThreshold >= 0:
            ...
            if data_len != 0:                           # Compression is enabled and the packet is compressed
                if data_len < compressionThreshold:     # Uncompressed length should be over the threshold
                    raise ProtocolError(
    ...
    def send(self, sock: socket.socket):
        ...
        if compressionThreshold >= 0:
            if len(buf) > compressionThreshold:   # Compression is enabled and the amount of data is over the threshold
        ...
        sock.send(types.VarInt.pack(len(buf)))  # Send packet length and the packet itself
        sock.send(buf)
