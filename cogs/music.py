...
class Music(commands.Cog):
    ...
    async def _summon(
        ...
        if not channel and not ctx.author.voice:
            raise VoiceError(
                "You aren't in a voice channel and you didn' specify one, come on man/woman/person!"
            )
    ...
    async def _skip(self, ctx: commands.Context):
        ...
        if voter == self.voice_state.current.requester:
            ...
            await ctx.send("You already want to!")
    ...
    async def _queue(self, ctx: commands.Context, *, page: int = 1):
        ...
        embed = discord.Embed(
            description="**{} tracks:**\n\n{}".format(
                len(self.voice_state.songs), queue
            )
    ...
    async def ensure_voice_state(self, ctx: commands.Context):
        if not ctx.author.voice or not ctx.author.voice.channel:
            raise commands.CommandError("You aren't in a channel")
