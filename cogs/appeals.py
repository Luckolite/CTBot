...
class Appeals(commands.Cog):
    def __init__(self, bot):
        ...
        if path.isfile(self.path):                      # load blacklist from file, if possible
    ...
    @commands.command(name='appeal',
                      description='Requests a ban appeal.')  # defines the command to be `appeal` and sets the description.
    ...
    async def appeal(self, ctx, *,
                     appeal):  # creates the ASYNC function for appeals, this is necessary for all bot commands.
        ...
        if user_id in self.blacklist:  # if the user id is in the blacklist
            ...
            cooldown = self.blacklist[user_id][
                'cooldown']  # gets the cooldown of the user if they are not banned from appeals
            if cooldown:  # if there is a cooldown
                ...
                if cd > 0:  # if the cooldown is larger than 0
                    ...
                    if cooldown >= 86000:  # if the cooldown is more than or equal to 86000
                        msg += f"{cooldown // 86000} day, "  # I think this is quite self explanatory
                    if cooldown >= 3600:  # if the cooldown is more than or equal to 3600
                        ...
                        if h % 10 != 1:  # modulo is %, self explanatory from there.
            ...
            e.description = "Appeals need to contain why you are banned, and a reason for being unbanned. Lack of " \
                            "either, or abuse of this command results in not being able to use the command anymore!"

            for text_group in [appeal[i:i + 1000] for i in
                               range(0, len(appeal), 1000)]:  # I really can not be bothered to explain this.
                e.add_field(name='◈ Your Appeal', value=text_group,
                            inline=False)  # add a field saying what is in `name=*` and value `value=*`
